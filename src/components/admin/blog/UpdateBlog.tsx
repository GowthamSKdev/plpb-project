...
const UpdateBlog = ({ setOpenUpdateBlog, HandleUpdate, blogToUpdate }) => {
  const blogData = blogToUpdate;
  const [formData, setFormData] = useState({
    title: blogData.title,
    description: blogData.description,
    category: blogData.category,
    tags: [],
    imageUrl: blogData.image_url || blogData.image_url,
    imageFile: blogData.image_url,
    content: blogData.content,
    htmlContent: blogData.html_content,
    postingDate: new Date().toISOString().split('T')[0],
  });

  const [showPreview, setShowPreview] = useState({ thumbnail: false, image: true });
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'content') {
      ...
      const htmlContent = paragraphs.map((p) => `<p>${p.trim()}</p><br/>`).join('\n');
  ...
  const HandleCreateBlogSubmit = (e) => {
    e.preventDefault();
    HandleUpdate(blogData._id, formData);
  };
      ...
      <div className="max-h-[80vh] w-full max-w-4xl overflow-auto rounded-2xl bg-white shadow-2xl">
        ...
        <div className="sticky top-0 rounded-t-2xl border-b border-gray-200 bg-white px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => setOpenUpdateBlog(false)}
        ...
        <form className="space-y-6 p-6" onSubmit={HandleCreateBlogSubmit}>
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-700">Blog Title *</label>
            <input
              type="text"
              name="title"
            ...
            {['image'].map((type) => (
              <div key={type} className="space-y-3">
                <label className="block flex items-center gap-1 text-sm font-semibold text-gray-700">
                  <Image size={16} /> {type === 'thumbnail' ? 'Thumbnail' : 'Featured'} Image
                </label>
                ...
                <div className="flex w-full items-center justify-center">
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-500">Click to upload {type}</p>
                    <input
                      type="file"
                      id={`${type}File`}
                      className="hidden"
                ...
                {showPreview[type] && (previewUrls[type] || formData[`${type}Url`]) && (
                  <img
